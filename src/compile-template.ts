...
// 替换 script 中的 变量
const replaceScriptVar = (code: string | null, regx: RegExp, options: Record<string, string>): string => {
  if(!code) return ''
...
const replaceStyleVar = (code: string | null, regx: RegExp, options: Record<string, string>): string => {
  if(!code) return ''
...
// 编译转换模板文件
const scriptStyleCompile = (source: string ): {
  script: SourceFn;
  style: SourceFn;
} => {
  // 获取模板内容
  ...
  return {
    script: (config: Record<string, string>) => replaceScriptVar(script, varRegx, config),
    style: (config: Record<string, string>) => replaceStyleVar(style, varRegx, config)
  }
}
...
export const compile = (compileConfig: CompileConfig): {
  style: SourceFn;
  script: SourceFn;
  template: SourceFn;
} => {
  ...
  const { script, style } = scriptStyleCompile(readFileSync(`${target}.vue`).toString())
